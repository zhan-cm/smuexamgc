...
public class QuestionGeneratorCharger {
    ...
    public static GeneratedQuestion sendMessage(String questionRequire, int promptJsonIndex, int retryTime, boolean useEnglish) {
        ...
        roleMessage.setContent("你是题目生成助手，我将给你一个题目要求，包含题目所属试卷/课程，题目主题词要求等要求，请你生成一道专业综合性较强且难度较大的题。"+
                "可从互联网搜题，你的回复内容应为json字符串格式，格式如下："+formatRequire+useEnglishStr);
        ...
        try (Response response = client.newCall(request).execute()) {
            ...
            if(ansMap.get("choices")!=null){
                ...
                GeneratedQuestion askResponse = JSONObject.parseObject(AIUtils.findJson(content), GeneratedQuestion.class);
                return askResponse;
            }
        } catch (IOException e) {
            e.printStackTrace();
    ...
    private static List<String> findQuestionTheme(String questionRequire, int retryTime) {
        ...
        aiPayload.setModel("ernie-4.5-turbo-32k");
        aiPayload.setSeed(Math.abs(random.nextInt()));
        aiPayload.getResponseFormat().setType("json_object");
