	...
	<table style="border-collapse:collapse; padding:0; margin:0;width: 100%">
		<tr>
			<td style="text-align: center">
				<h3 id="repeatTitle">《${ename}》重复试题（仅查题目）</h3>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$('#datalist').datagrid({
			...
			striped: true,
			singleSelect: false,
			url:'${pageContext.request.contextPath}/paper/findRepeatQuestions',
			pagination: true,
			rownumbers: false,
			pageSize: pageSize,
			...
			columns:[[
				...
				{field:'content',title:'题目',width:80,align:'left',formatter:function(value,row,index){
						...
						s+= '<a href="javascript:void(0);" class="previewQuestion" onclick="getQuestionDetail('+row.qid+','+ row.mqid +',' + row.ismain +',' + row.qtiscon + ')">'+row.content+'</a>';
						s +='</div>';
						return s;
					}},
				...
				{field:'opration',title:'操作',width:40,align:'center',formatter:function(value,row,index){
						var s1 = '<a href="javascript:void(0);" class="viewQuestion easyui-tooltip" onclick="getQuestionDetail('+row.qid+','+ row.mqid+ ',' + row.ismain +',' + row.qtiscon + ')" title="查看详情"></a>';
						var s2 = '<a href="javascript:void(0);" class="editcls1 easyui-tooltip" onclick="gotoEditQuestion('+row.qid+','+ row.mqid+ ',' + row.ismain +',' + row.qtiscon + ')" title="编辑试题"></a>';
	...
	function getQuestionDetail(qid,mqid,ismain,qtiscon){
		if(!mqid || typeof (mqid)===undefined ||mqid==="null"){
			mqid = '';
		}
		openIframeDialog({
			url:"${pageContext.request.contextPath}/paper/previewQuestion?qid="+qid+"&mqid="+mqid+"&eid="+eid+"&isMain="+ismain+"&iscon="+qtiscon+"&pageNumber="+$('#datalist').datagrid('getPager').pagination('options').pageNumber+"&pagesize="+$('#datalist').datagrid('getPager').pagination('options').pageSize+"&repeat=1",
			fit:true,
			title:'查找重复试题'
		},0);
	}
	...
	function gotoEditQuestion(qid,mqid,ismain,qtiscon){
		openIframeDialog({
			url:"${pageContext.request.contextPath}/question/editQuestion?q_id="+qid+"&mqid="+mqid+"&eid="+eid+"&isMain="+ismain+"&iscon="+qtiscon,
			fit:true,
			title:'修改试题'
		},1);
	}
	...
	function questionFilter(){
		...
		if($('#sameType').val()==0){
			...
			$('#repeatTitle').text('《'+$('#ename').val()+'》重复试题（仅查重题目）');
		}else{
			$('#selectSameBtn').find('.l-btn-text').text('选中重复试题（题目与答案同时相同）');
			$('#repeatTitle').text('《'+$('#ename').val()+'》重复试题（题目与答案同时相同）');
		}
		var p = $('#datalist').datagrid('getPager');
	...
	function getThemeList(th_level, th_pid){
		$.ajax({
			...
			async: false,
			type: "POST",
			data: {"th_level": th_level, "th_pid": th_pid, "eid": eid},
	...
	function del(qid, ismain,iscon){
		$.ajax({
			...
			async: false,//改为同步方式
			type: "POST",
			data: { "q_id":qid ,"isMain":ismain,"iscon":iscon,"ei_id":eid},
			success: function (data) {
				toastr.success("删除成功");
				$('#datalist').datagrid('reload');
			}
				...
				fn:function(r){
					if(r){
						...
						for(let i=0;i<rows.length;i++){
							...
							param["eid"] = rows[i].eid;
							list.push(param);
						}
						let data = {data:list,eid:eid};
						$.ajax({
