...
<div style="text-align: center;">
  <table id="optionTable" style="width: 100%;text-align: center;border: none;">
    ...
    <tr>
      <td style="border: none;">
        <input id="saveAsWord" type="button" value="保存为word（当前浏览样式样式）" onclick="saveAsWord()"/>
        <input id="saveAsWord_hj" type="button" value="保存为word（定制试卷模板）" onclick="saveAsWord_hj()"/>
      </td>
    </tr>
    <tr>
      <td style="border: none;">
...
<script type="text/javascript" charset="utf-8" src="${pageContext.request.contextPath}/styles/html2canvas/purify.min.js"></script>
<script type="text/javascript">
  var eid = $("#eid").val();
  var examDate = "${examdate}";
  var examObject = "${examObject}";
  var res = <%= request.getAttribute("resJson") %>;
  var etime = "${etime}";
  var questiontype = <%= request.getAttribute("questiontypeJson") %>;
  var examInfo = <%= request.getAttribute("examInfoJson") %>;
  var showAns = ${aFlag==1};
  // 获取当前设备的像素密度比
  const dpr = window.devicePixelRatio || 1; // 默认为 1
  const pxToPtRatio = 1.333; // 假设标准 DPI 为 96（即 1pt = 1.333px）
  const conversionFactor = dpr * pxToPtRatio;
  ...
  /**
   * 从字符串中提取 <sub>...</sub> 和 <sup>...</sup>（大小写不限）标签的内容，
   * 并将标签内外的内容分割成不同的对象。
   * 兼容 <sub class="xx" style="xx"> 这类带属性的场景。
   *
   * @param {string} input - 包含<sub></sub>或<sup></sup>标签的字符串
   * @returns {Array<{type: "normal" | "sub" | "sup", str: string}>}
   */
  function parseSubSupTags(input) {
    const result = [];
    const re = /<(sub|sup)\b[^>]*>([\s\S]*?)<\/\1>/gi;
    let lastIndex = 0;
    let match;
    while ((match = re.exec(input)) !== null) {
      // match.index 是本次匹配在原字符串中的起始位置。
      // 先将标签之前的普通文本推入数组
      const textBefore = input.slice(lastIndex, match.index);
      if (textBefore) {
        result.push({
          type: "normal",
          str: stripHtml(textBefore)
        });
      }
      // match[1] 是匹配到的标签名 (可能是 sub, SUB, sup, SUP 等)
      // 这里把它转成小写，以便统一
      const tagName = match[1].toLowerCase();
      // match[2] 是标签内的具体内容
      result.push({
        type: tagName, // "sub" or "sup"
        str: stripHtml(match[2])
      });
      // 更新 lastIndex，跳到这次匹配的末尾，以便继续往后匹配
      lastIndex = re.lastIndex;
    }
    // 匹配完后，如果尾部还有普通文本，就继续推入数组
    const textAfter = input.slice(lastIndex);
    if (textAfter) {
      result.push({
        type: "normal",
        str: stripHtml(textAfter)
      });
    }
  ...
  function stripHtml(content) {
    ...
    // 2) 用 DOMPurify 清理富文本
    //    这里允许 br/p 保留，便于后面转成 Word 换行
    //    （如果你确定不需要 p，也可以只保留 br）
    ...
    // 4) 把 br/p 转换为占位符
    clean = clean
            .replace(/<br\s*\/?>|<\/br>/gi, "___BR___PLACEHOLDER___")
            .replace(/<p[^>]*>/gi, "___P___START___")
            .replace(/<\/p>/gi, "___P___END___");

    // 5) 理论上 DOMPurify 后只剩少量允许标签残片
    //    用更安全的“真实标签”正则清理一下
    ...
    // 8) 占位符还原为 Word 换行
    clean = clean
            .replace(/___BR___PLACEHOLDER___/g, "<w:br/>")
            .replace(/___P___START___/g, "")
            .replace(/___P___END___/g, "<w:br/>");

    // 9) 你原先的双换行去重逻辑保留
    clean = clean.replace("<w:br/><w:br/>", "<w:br/>")
            .replace("<w:br/><w:br/>", "<w:br/>");

    while (clean.startsWith("<w:br/><w:br/>")) {
      clean = clean.slice(7);
    }
    while (clean.endsWith("<w:br/><w:br/>")) {
  ...
  function calculateWrapNum(qtname, atid){
    let answerType = parseInt(atid);
    let wrapNum = 1;
    if(answerType<=5 || answerType===8 || answerType===9 || answerType===10 || qtname==="填空题"){ //单选多选判断、（填空题也不需要？）
      wrapNum = 0;
    }else if(answerType===6 || answerType===7 || answerType===11){
      wrapNum = 4;
      if(qtname){
        if(qtname.includes("简答")
                || qtname.includes("分析")
                || qtname.includes("论述")
                || qtname.includes("作文")
                || qtname.includes("写作")
   ...
   * @returns {Promise<string>} 返回base64（不包含data:image/png;base64,前缀），图片长宽（pt单位）
   */
            ...
            .then(blob => {
              return new Promise((resolve, reject) => {
                ...
                img.onload = () => {
                  ...
                  const width = (img.width / conversionFactor).toFixed(2);
                  const height = (img.height / conversionFactor).toFixed(2);
  ...
  /**
   * 处理包含img标签的字符串，提取文本和img的base64
   * @param {string} inputStr 输入字符串
   * @returns {Promise<{contentList: string[], imgList: string[]}>}
   */
  async function processString(inputStr,contentListList,imgListList, filepath) {
    let contentList = [];
    let imgList = [];
    // <img src="xxx"> 或 <img src='xxx'> 均可匹配
    const imgRegex = /<img\s+[^>]*src=(["'])(.*?)\1[^>]*>/gi;
    let match;
    let currentTextStart = 0;

    while ((match = imgRegex.exec(inputStr)) !== null) {
      const fullImgTag = match[0];
      const imgSrc = match[2];
      const imgTagIndex = match.index;
      const textBeforeImg = inputStr.slice(currentTextStart, imgTagIndex);
      // 尝试解析 width 与 height 属性
      const widthMatch = fullImgTag.match(/\bwidth\s*=\s*["'](\d+)["']/i);
      const heightMatch = fullImgTag.match(/\bheight\s*=\s*["'](\d+)["']/i);
      let specifiedWidth = widthMatch ? (parseInt(widthMatch[1], 10) / conversionFactor).toFixed(2) : null;
      let specifiedHeight = heightMatch ? (parseInt(heightMatch[1], 10) / conversionFactor).toFixed(2) : null;
      try {
        const base64Data = await imageToBase64(imgSrc);
        // 若 width 或 height 未指定，则使用实际图片大小
        const finalWidth = specifiedWidth || base64Data.width;
        const finalHeight = specifiedHeight || base64Data.height;
        if (textBeforeImg) {
          contentList.push(parseSubSupTags(textBeforeImg));
        }
        allImgList.push({
          base64 : base64Data.base64,
          questionLabel : questionLabelCount
        });
        imgList.push({
          questionLabel : questionLabelCount++,
          width : finalWidth,
        ...
        currentTextStart = imgTagIndex + fullImgTag.length;
      } catch (e) {
        const failedPart = inputStr.slice(currentTextStart, imgTagIndex + fullImgTag.length);
        if (contentList.length > 0) {
          const lastIndex = contentList.length - 1;
          contentList[lastIndex] = contentList[lastIndex] + failedPart;
        } else {
          contentList.push(parseSubSupTags(failedPart));
        }
        currentTextStart = imgTagIndex + fullImgTag.length;
      }
    }

    // 处理最后剩余的文本（最后一个img标签后的文本）
    if (currentTextStart < inputStr.length) {
      const lastText = inputStr.slice(currentTextStart);
      if (lastText) {
        contentList.push(parseSubSupTags(lastText));
      }
    }

    if(filepath && /\.(png|jpe?g|svg|bmp|gif|heic|webp|tiff)$/i.test(filepath)){
      try {
    ...
    contentListList.push(contentList);
    imgListList.push(imgList);
    return { contentList, imgList };
  }

  var contentListList = [];
  var imgListList = [];
  var answerContentListList = [];
  var answerImgListList = [];
  var allImgList = [];
  var questionLabelCount = 63;//图片标签，用于word标记图片，随机一个数即可，不可过大

  async function saveAsWord_hj(){
    let questionsByQt = [];
    contentListList = [];
    imgListList = [];
    answerContentListList = [];
    answerImgListList = [];
    questionLabelCount = 63;
    allImgList = [];
    for(let i=0;i<res.length;i++){
      await processString(res[i].content, contentListList,imgListList,res[i].filepath);
      if(res[i].ismain===0){
        if(showAns){
          await processString(res[i].rightAns, answerContentListList, answerImgListList,null);
        }else{
          answerContentListList.push(parseSubSupTags(res[i].rightAns));
          answerImgListList.push([]);
        }
      }else{
        answerContentListList.push([]);
        answerImgListList.push([]);
      }
    }
    ...
    for(let i=0;i<resClone.length;i++){
      if (resClone[i].iscon === 1){
        if(resClone[i].ismain === 1){
          myQuestion[questionCount] = {
            th : resClone[i].th,
            qtype : resClone[i].qtype,
            iscon : resClone[i].iscon,
            ismain : resClone[i].ismain,
            atid : resClone[i].atid,
            content : contentListList[i],
            imgList : imgListList[i],
            questionList : []
          };
          for(let j=0;j<resClone.length;j++){
            if(resClone[j].mqid === resClone[i].qid){
              if(parseInt(resClone[j].th) < parseInt(myQuestion[questionCount].th)){
                myQuestion[questionCount].th = resClone[j].th;
              }
              myQuestion[questionCount].questionList.push({
                th : resClone[j].th,
                iscon : resClone[j].iscon,
                ismain : resClone[j].ismain,
                atid : resClone[j].atid,
                content : contentListList[j],
                imgList : imgListList[j],
                rightAns : answerContentListList[j],
                answerImgList : answerImgListList[j]
              });
            }
          }
          questionCount++;
        ...
        myQuestion[questionCount++] = {
          ...
          content : contentListList[i],
          imgList : imgListList[i],
          rightAns : answerContentListList[i],
          answerImgList : answerImgListList[i]
        };
      }
    }

    let hasOverTenLettersWordCount = 0;//超长题型字符串数量
    let examinfoScore = 0;
    for(let j=0;j<questiontype.length;j++){
      if (typeof questiontype[j].QTNAME === 'string' && questiontype[j].QTNAME.length > 10) {
        hasOverTenLettersWordCount++;
      }
      examinfoScore += questiontype[j].SCORE;
      let qt = {
        qtname : questiontype[j].QTNAME,
        atid : questiontype[j].ATID,
        qscore : questiontype[j].QSCORE,
        score : questiontype[j].SCORE,
        wrapNum : calculateWrapNum(questiontype[j].QTNAME,questiontype[j].ATID),
        qtypeNum : NumberToChinese(j+1),
        question : []
      };
      let qtQuestionCount = 0;
      for(let i=0;i<myQuestion.length;i++){
        if(myQuestion[i].qtype === questiontype[j].QTID){
          qt.question[qtQuestionCount++] = myQuestion[i];
        }
      }
      questionsByQt.push(qt);
    }

    let hasOverTenLettersWord = false;//判断有没有超长题型字符串
    if(questiontype.length>=4 && hasOverTenLettersWordCount>3){
      hasOverTenLettersWord = true;
    }else if(questiontype.length>=5 && hasOverTenLettersWordCount>2){
      hasOverTenLettersWord = true;
    }else if(questiontype.length>=6 && hasOverTenLettersWordCount>1){
      hasOverTenLettersWord = true;
    }

    let params = {};
    params.allImgList = allImgList;
    params.examDate = examDate;
    params.examObject = examObject;
    params.questionsByQt = questionsByQt;
    params.etime = etime;
    params.showAns = showAns;
    params.qtTableFontSize = 30; //题型表格字体大小
    if(questionsByQt.length>=8 || hasOverTenLettersWord){
      params.qtTableFontSize = 24;
    }
    params.examInfo = {
      ename : examInfo.ENAME,
      aorb : examInfo.AORB===0?"A":"B",
      qtypeNum : NumberToChinese(questionsByQt.length),
      score : examinfoScore,
      unitName : examInfo.UNITNAME,
    };
    //console.log(params);
    downloadFile(params, '${pageContext.request.contextPath}/viewPaper/exportHaijunPaper');
  }

  function downloadFile(postJsonData, url) {
    ...
    xhr.onload = function () {
      if (xhr.status === 200) {
        ...
        link.download = "《"+examInfo.ENAME+"》试卷模板.doc";
    ...
    xhr.send(JSON.stringify(postJsonData));
  }
