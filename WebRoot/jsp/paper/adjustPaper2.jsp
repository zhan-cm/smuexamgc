...
<form id="adjustForm" method="post" action="${pageContext.request.contextPath}/paper/setQuestionType2">
...
$(".second").on('input',function(){
	...
	$(this).prev().find("option[value='"+val+"']").attr("selected", true);;
})
...
function submitForm(){
	...
	if(res==null||res.length==0||isNull == 'false'){
		...
		$.ajax({
            url: "${pageContext.request.contextPath}/paper/setQuestionType2",
            async: true,//改为同步方式
            type: "POST",
            traditional: true,
            data: { "ei_id":$("#ei_id").val(),"c_id": $("#c_id").val(),"res":res},
...
function cancel(){
	var b = true;
	$(".qtscore").each(function(){
		if($(this).val()==""){
			b = false;
		}
	})
	if(b==false){
		toastr.error("尚有题型未赋分，请赋分后再关闭！");
	}else{
