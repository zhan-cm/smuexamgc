...
<table cellpadding="0" cellspacing="0" style="width:100%">
	<tr>
		<td style="padding-left:2px">
			<a href="javascript:void(0);" onclick="javascript:history.go(-1);" class="easyui-linkbutton" data-options="iconCls:'icon-back',plain:true">返回</a>
	...
	<tr>
		<td>
			不选用<input type='text' id='forbidDay' name='forbidDay' style='width:50px' value='${applicationScope.question_used_day}'/>天之内考过的试题，不选用使用过<input type='text' id='forbidNum' name='forbidNum' style='width:50px' value='${applicationScope.question_use_time}'/>次及以上的试题，
			<select id="isVerified" name="isVerified"  class="qlimit" onchange="questionFilter()">
...
function addQuestionIntoPaper(){
	...
	for(var i=0; i<rows.length; i++){
		res.push(rows[i].qid+"_"+rows[i].eid+"_"+rows[i].qtid+"_"+rows[i].ismain);
	}
	...
	$.ajax({
        url: "${pageContext.request.contextPath}/paper/addQuestionIntoPaperFromPaper",
        async: true,//改为同步方式
        type: "POST",
        traditional: true,
        data: { "qids": uniqueArray(res), "c_id" : cid, "ei_id" : eid},
...
function adjustPaper(){
	$.ajax({
		url: "${pageContext.request.contextPath}/paper/updateExaminfoCids",
		type: "POST",
		success: function(data){
			window.location.href = '${pageContext.request.contextPath}/paper/adjustPaper?c_id=' + cid + '&ei_id=' + eid+ '&way=' + state;
		}
	})
}*/
...
function getQuestionDetail(eid_,qid,mqid,ismain,iscon,atid,edit){
	...
	openIframeDialog({
		url:"${pageContext.request.contextPath}/paper/previewQuestion?qid="+qid+"&mqid="+mqid+"&isMain="+ismain+"&iscon="+iscon +"&eid="+eid_+"&edit="+edit+"&atid="+atid,
		fit:true,
		title:'查看试题'
	},0);	
}
