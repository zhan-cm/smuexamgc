...
<input type="hidden" id="rebuild" value="${rebuild}"/>
...
<table style="border:none; width: 100%; border-collapse: collapse; border-spacing: 0; margin: 5px;">
	<tr>
		<th style="text-align:center;font-size: 20px;margin: 5px;">试卷： ${ename} （B卷）（试卷编号：${ei_id}）</th>
	</tr>
</table>
	...
	<tr>
		<td style="padding-left:2px">
			...
			<c:if test="${isAdmin=='y'||readonly=='0'}">
				...
				<a href="javascript:void(0);" onclick="adjustPaper('${c_id}','${ei_id}','1')" class="easyui-linkbutton" data-options="iconCls:'icon-book_edit',plain:true" >按题型调整顺序分值时间</a>&nbsp;&nbsp;
				<a href="javascript:void(0);" onclick="score('${c_id}','${ei_id}')" class="easyui-linkbutton" data-options="iconCls:'icon-book_addresses_edit',plain:true" >逐题赋分赋时</a>&nbsp;&nbsp;
				<a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-arrow_undo',plain:true" onclick="rebuild('${c_id}', ${ei_id})">重新组卷</a>&nbsp;&nbsp;
				<a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-book_tabs',plain:true" onclick="toFindRepeatQuestions('${ei_id}');">查找重复试题</a>
			</c:if>			
			<a href="javascript:void(0);" onclick="questionTypeExplain('${ei_id}')" class="easyui-linkbutton" data-options="iconCls:'icon-book_open',plain:true" >题型说明</a>&nbsp;&nbsp;
			<a href="javascript:void(0);" onclick="checkList('${c_id}','${ei_id}');" class="easyui-linkbutton" data-options="iconCls:'icon-asterisk_orange',plain:true" >双向细目表</a>&nbsp;&nbsp;	
			<a href="javascript:void(0);" onclick="forecastPaper('${c_id}','${ei_id}')" class="easyui-linkbutton" data-options="iconCls:'icon-asterisk_red',plain:true" >预测分析</a>&nbsp;&nbsp;
			<a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-asterisk_yellow',plain:true" onclick="seePaper(${ei_id})">预览试卷</a>&nbsp;&nbsp;
			<a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-bookmark',plain:true" onclick="viewExamInfo('${c_id}','${ei_id}')">查看考务信息</a>&nbsp;&nbsp;
			<a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-bookmark_edit',plain:true" onclick="inEditExamInfo('${c_id}','${ei_id}')">编辑考务信息</a>&nbsp;&nbsp;
			<c:choose>
			...
			<a href="javascript:void(0)" onclick="appoint('${c_id}','${ei_id}','${ename}')" class="easyui-linkbutton" data-options="iconCls:'icon-book_key',plain:true">试卷权限设定</a>
			<a href="javascript:void(0);" onclick="adjustQuestionOrder('${c_id}','${ei_id}','1')" class="easyui-linkbutton" data-options="iconCls:'icon-book_edit',plain:true" >调整试题顺序</a>
			<a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-computer_edit',plain:true" onclick="testPaper(${ei_id})">模拟测试</a>
			<c:if test="${isAdmin=='y'||readonly=='0'}">
				<a href="javascript:boid(0)" class= "easyui-linkbutton" data-options="iconCls:'icon-delete',plain:true" onclick="delSelect();">删除选中的试题</a>
			</c:if>
		</td>
	</tr>
</table>
...
<table>
	<tr>
		<td>
		 	<font color="blue" style="font-size:18px;">本试卷总分值：<span style="font-size:18px;" id="sum">0</span>分，共<span style="font-size:18px;" id="total">0</span>题，试题总用时：<span style="font-size:18px;" id="time">0秒</span></font>&nbsp;&nbsp;&nbsp;&nbsp;<font style="font-size:18px;" color="red">与A卷共有<span style="font-size:18px;" id="repeNum">0</span>道相同试题，相同率：<span style="font-size:18px;" id="repeRate">0</span>%（题号前带*号表示此题已出现在A卷中）</font>
		</td>		
	</tr>
</table>
</div>
...
$(document).ready(function(){
 	if($('#firstVerify').val()!=''){
 		toastr.warning("提示：等待初审的试卷从题库加题或从已有试卷加题，试卷需要重新提交审核");
 	}
 	
 	var rebuild = $('#rebuild').val();
 	if(rebuild!=''){
 		toastr.success("已成功重新组卷！");
 	}
	...
	$('#datalist').datagrid({
		...
		columns:[[
			  ...
			  {field:'content',title:'题目',width:145,align:'left',formatter:function(value,row,index){
				  ...
				  var s = '<a href="javascript:void(0);" class="previewQuestion" onclick="getQuestionDetail(\''+row.qid+'\',\''+ row.mqid+ '\',\''+ row.version+ '\',\'' + row.ismain +'\',\'' + row.iscon + '\',\'' + edit + '\',\'' + row.atid + '\')"><div class="wrap">'+content+'</div></a>';
		    	  return s;                                                               
	          }},
	          {field:'answer',title:'答案',width:120,align:'left',formatter:function(value,row,index){
	        	  var ismain=row.ismain;
				  	if(ismain==1){
				  		...
				  		if(row.atid == 13){
							var s = '<a href="javascript:void(0);" class="previewQuestion" onclick="getQuestionDetail('+row.qid+','+ row.mqid+','+','+row.version+','+ row.ismain +',' + row.iscon +',' + edit + ',' + row.atid +')">' +
			...
			{field:'use_time', hidden:${openFindRepeatSystem eq 0}, title:'已考次数<br >(三年内)',width:25,align:'left',formatter:function(value,row,index){
	          ...
	          {field:'opration',title:'操作',width:70,align:'center',formatter:function(value,row,index){
	        	  ...
	        	  //var s1 = "<a href=\"${pageContext.request.contextPath}/question/previewQuestion?qid="+row.qid+"&mqid="+mqid+"&version="+row.version+"&c_ids="+c_id+"&isMain="+row.ismain+"&iscon="+row.iscon +"&eid="+ei_id+"&isB="+isB+"\" class=\"buttons\">浏览</a>&nbsp;&nbsp;&nbsp;&nbsp;";
		          //var s2 = "<a href=\"${pageContext.request.contextPath}/question/editQuestion?q_id="+row.qid+"&mqid="+mqid+"&version="+row.version+"&c_ids="+c_id+"&isMain="+row.ismain+"&iscon="+row.iscon +"&eid="+ei_id+"&isB="+isB+"\" class=\"buttons\">编辑</a>&nbsp;&nbsp;&nbsp;&nbsp;";
	        	  var s1 = '<a href="javascript:void(0);" class="buttons" onclick="getQuestionDetail(\''+row.qid+'\',\''+ row.mqid+ '\',\''+ row.version+ '\',\'' + row.ismain +'\',\'' + row.iscon + '\',\'' + edit + '\',\'' + row.atid + '\')">浏览</a>&nbsp;&nbsp;&nbsp;&nbsp;';
	        	  var s2 = '';
	        	  if(edit==''){
...
//如果是B卷，查询与A卷的试题重复数
function getRepetition4AB(){
	$.ajax({
		url: "${pageContext.request.contextPath}/paper/findTwoPaperRepeatQuestions",
		async: true,
		type: "POST",
		data: { "eid1":$("#ei_id").val(), "eid2": "${bid}" , "answerMatch": 1},
		success: function (data) {
			if(total>0 && Array.isArray(data.repeatList)){
				let percentage = (data.repeatList.length / total) * 100;
				$("#repeRate").html(percentage.toFixed(2));
				$("#repeNum").html(data.repeatList.length);
			}
			let repeList = data.repeatList;
			let rows = $("#datalist").datagrid("getRows");
			for(let i=0;i<rows.length;i++){
				for(let j=0;j<repeList.length;j++){
					if(rows[i].qid==repeList[j].fromQid1){
						rows[i].th = "<a style='color:red;font-size:20px;'>*"+rows[i].th+"</a>";
...
function rebuild(cid, eid){
	$.messager.confirm("提示",'重新组卷将删除此试卷原所有的试题和考生的作答记录，确定继续 ?',function(r){
	    if (r){
	    	//updateSelectTab();
	    	window.location.href='${pageContext.request.contextPath}/paper/rebuild?c_id=' + cid + '&ei_id=' + eid;
	    }
	});
}
...
function inEditExamInfo(cid,eid){
	var url = "${pageContext.request.contextPath}/paper/inEditExamInfo?cid="+cid+"&eid="+eid;
	/*
	var content = '<iframe frameborder="no" width="100%" height="100%" src="'+url+'" scrolling="auto" style="overflow:auto"></iframe>';
	window.parent.$('#nav_tab').tabs('add',{
		title:'编辑考务信息',
...
function viewExamInfo(cid,eid){
	var url = "${pageContext.request.contextPath}/paper/viewExamInfo?cid="+cid+"&eid="+eid;
	/*var content = '<iframe frameborder="no" width="100%" height="100%" src="'+url+'" scrolling="auto" style="overflow:auto"></iframe>';
	window.parent.$("#nav_tab").tabs('add',{
		title:'查看考务信息',
		content: content,
...
function getQuestionDetail(qid,mqid,version,ismain,iscon,edit,atid){
	if(typeof(mqid) == "undefined"){
		mqid = "";
	 }
	 openIframeDialog({
		url:"${pageContext.request.contextPath}/paper/previewQuestion?qid="+qid+"&mqid="+mqid+"&version="+version+"&c_ids="+c_id+"&isMain="+ismain+"&iscon="+iscon +"&eid="+ei_id+"&isB="+isB+"&edit="+edit+"&atid="+atid,
		fit:true,
		title:'查看试题'
	},0);	
}

function gotoEditQuestion(qid,mqid,version,ismain,iscon){
	if(typeof(mqid) == "undefined"){
		mqid = "";
	}
	openIframeDialog({
		url:"${pageContext.request.contextPath}/paper/editQuestion?q_id="+qid+"&mqid="+mqid+"&version="+version+"&c_ids="+c_id+"&isMain="+ismain+"&iscon="+iscon +"&eid="+ei_id+"&isB="+isB,
		fit:true,
		title:'编辑试题'
	},1);
...
function appoint(cid,eid,ename){
	var params = {};
	params["eid"] = eid;
	$.ajax({
		  contentType: "application/json; charset=utf-8",
          ...
          success: function (data) {
          	...
          	/*
				...
				openIframeDialog({
					url:"${pageContext.request.contextPath}/verify/appoint?cid="+cid+"&eid="+eid+"&ename="+ename,
					fit:true,
					title:'试卷权限设置'
				},0);
          	}else if(data==2){
...
function forecastPaper(cid,eid){
	openIframeDialog({
		url:"${pageContext.request.contextPath}/paper/forecastPaper?c_id="+cid+"&ei_id="+eid,
		fit:true,
		title:'预测分析'
	},0);
}

function score(cid,eid){
	openIframeDialog({
		url:"${pageContext.request.contextPath}/paper/score?c_id="+cid+"&ei_id="+eid,
		fit:true,
		title:'逐题赋分'
	},1);
}

function questionTypeExplain(eid){
	openIframeDialog({
		url:"${pageContext.request.contextPath}/paper/questionTypeExplain?eid="+eid,
		fit:true,
		title:'题型说明'
	},0);
}

function adjustPaper(cid,eid,way){
	openIframeDialog({
		url:"${pageContext.request.contextPath}/paper/adjustPaper?c_id="+cid+"&ei_id="+eid+"&way="+way+"&isB=1",
		fit:true,
		title:'按题型调整顺序分值时间'
	},1);
}
...
function printAnswer(rs,aid){
	var p = '';//选择题ABCDE
	var out = '';
	if(typeof(aid)=='undefined'||aid==''||aid=='null'||aid==null){
		return '';
	}else{
        var aid = aid.split(",");
...
function next(){
	...
	if(state==0){
		...
		titles.eq(tab_index).text('等待初级审核的试卷');
		window.location.href = "${pageContext.request.contextPath}/verify/waitFirstVerify";
	}else if(state==2){
		titles.eq(tab_index).text('等待终极审核的试卷');
		window.location.href = "${pageContext.request.contextPath}/verify/waitLastVerify";
	}
	
}

function toFindRepeatQuestions(eid){
	openIframeDialog({
		url:"${pageContext.request.contextPath}/paper/toFindRepeatQuestions?eid="+eid,
		fit:true,
		title:'查找重复试题'
	},0);
}
...
function adjustQuestionOrder(cid,eid,way){
	openIframeDialog({
		url:"${pageContext.request.contextPath}/paper/adjustQuestionOrder?c_id="+cid+"&ei_id="+eid+"&way="+way,
		fit:true,
		title:'调整试题顺序'
	},1);
}
