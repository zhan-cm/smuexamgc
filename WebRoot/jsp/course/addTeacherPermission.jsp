...
<div id="dlg-toolbar" style="height:auto;">
	<table style="border-collapse:collapse; padding:0; margin:0;width: 100%">
		<tr>
			<th style="text-align:center;font-size: 20px;margin: 5px;">《${cname}》新增教师权限</th>
		</tr>
	</table>
	<table>
		<tr>
...
$(document).ready(function(){
	$('#datalist').datagrid({
		...
		singleSelect: false,
		checkOnSelect:false,
		url:'${pageContext.request.contextPath}/course/getAllTeacherPer?cid='+cid,
		pagination: true,
		rownumbers: false,
		pageNumber: 1,
		pageSize: 100,
		...
		columns:[[
		      ...
		      {field:'username',title:'用户名',width:20,align:'left',sortable:true},
	          ...
	          {field:'permission',title:'权限',width:100,formatter:function(value,row,index){
	        	  var cp = per.cp;
	        	  var qp = per.qp;
	        	  var pp = per.pp;
	        	  var s1 = '课程';
	    ...
	    onLoadSuccess:function(data){
	    	$.each($(":checkbox[name=allPer]"),function(i,item){
				var rid = $(".perList").eq(i).find(".rid").val();
				var defPer = getPermissionsByRID(rid);
				
				$(this).on("click",function(){					
					if($(this).attr('checked')){
						$.each(($(".perList").eq(i).find(".per")),function(i,it){
							var b = false;
							var perid = $(it).val();
							for(var i=0;i<defPer.length;i++){
								if(defPer[i].id==perid){
						...
						$(".perList").eq(i).find("input[type=checkbox]").prop("checked",false);
					}
				})
			})
	    }
...
function savePermissions(){
	...
	$.ajax({
		...
		url: "${pageContext.request.contextPath}/course/editCoursePermission_teachingoffice",
		async: false,
		type: "POST",
